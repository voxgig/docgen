...
export function proseText(source: string, format = 'md'): string {
  if (format === 'html' || format === 'vue') {
    ...
    return md.utils.unescapeAll(source).replace(/[ \t]+/g, ' ').replace(/ *\n */g, '\n').replace(/\n{3,}/g, '\n\n').trim()
  }
  ...
  return collect(md.parse(source, {})).replace(/[ \t]+/g, ' ').replace(/ *\n */g, '\n').replace(/\n{3,}/g, '\n\n').trim()
}
export function checkText(source: string, format = 'md', rejectFile = Path.join(PACKAGE, 'qa/styles/config/vocabularies/Docgen/reject.txt')): string[] {
  ...
  if (/\b(I|me|my|mine|we|us|our|ours)\b/i.test(text)) errors.push('Use neutral or second-person prose')
...
export function runQA(manifestPath: string, root = process.cwd(), vale = true): { files: number, errors: string[] } {
  ...
  try {
    ...
    if (vale) {
      const result = spawnSync('vale', ['--config=' + config, '--minAlertLevel=error', ...inputs], { encoding:'utf8', cwd:root })
