...
function summary(v) {
    ...
    if (first && server && !/[{}]/.test(server)) {
        ...
        lines.push('A read request without required parameters or authentication is ' + (0, exports.code)(first.point.m + ' ' + first.point.o) + '. For example:', fence('curl --fail-with-body --silent --show-error ' + "'" + url.replace(/'/g, "'\\''") + "'", 'sh'), 'Inspect the response using the ' + apiLink(first.entity) + ' reference. This checks the public route; authenticated operations need their own credentials and request data.', '');
    }
...
function fieldsTable(fields) {
    return ['| Field | Type | Required | Description |', '| --- | --- | --- | --- |',
        ...Object.values(fields).filter(f => f.a !== false).map(f => '| ' + (0, exports.code)(f.n) + ' | ' + (0, exports.code)(String(f.t || 'any').replace(/[`$]/g, '').toLowerCase()) + ' | ' +
            (f.r ? 'Yes' : 'No') + ' | ' + (0, exports.cell)(f.sh || f.h || '') + ' |')].join('\n');
}
function operationFacts(point, resolved) {
...
function routesOf(entity, resolved) {
    return rows(entity.op).flatMap((op) => (op.points ?? []).filter((p) => p.a !== false).map((point) => {
...
function pages(v, examples = {}) {
    ...
    add('guides/first-call', 'Make your first API call', 'Guides', first ?
        '1. Choose an SDK from the SDK section and follow its installation instructions.\n' +
            ...
            'The first endpoint is ' + (0, exports.code)(first.p.m + ' ' + first.p.o) + '. Request and response examples come from the API specification.' :
...
function slideBodies(v, example = '') {
    ...
    if (v.entities.length) {
        ...
        if (overflows)
            lines.push('- and ' + (v.entities.length - shown.length) + ' more, in the API reference');
    ...
    if (primary && example) {
        ...
        if (first) {
            ...
            chunks.push('Step 3: call an operation\n\n' + call + '\n\n' +
                'That reaches ' + (0, exports.code)(first.p.m + ' ' + first.p.o) + '. The reference page for ' +
