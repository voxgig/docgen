...
<div class="flex flex-col overflow-y-auto p-4 w-full">
  ...
  each(build, (lg: any) => {
    ...
    <section id="section-get-start-${spec.name}" class="lg:my-20 md:p-6 rounded-lg shadow-lg">
      ...
      <pre class="p-2 rounded-md"><code class="language-${spec.name}">`); spec.init(model, option); Content(`</code></pre>
          </section>
        </section>
  </section>`)
  })
...
<div class="w-full md:w-3/4 mx-auto p-6">
  ...
  <section id="${entity.Name}" class="p-6 rounded-lg my-30">
    ...
    `)
      ...
      <pre class="p-2 rounded-md"><code class="language-${spec.name}">`); spec.remove(op, entity); Content(`</code></pre>
      </section>
                  `)
        } else {
          Content(`
